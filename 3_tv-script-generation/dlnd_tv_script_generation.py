...
def token_lookup():
    ...
    """
    # TODO: Implement Function
    punc_to_token = {'.':'||PERIOD||', ',':'||COMMA||','"':'||QUOTATION_MARK||', ';':'||SEMICOOLON||',                      '!':'||Exclamation_Mark||', '?':'||QUESTION_MARK||', '(':'||LEFT_PARENTHESES||',                       ')':'RIGHT_PARENTHESES', '--':'DASH', '\n':'RETURN'}
...
def get_batches(int_text, batch_size, seq_length):
    ...
    for ii in range(n_batches):
        pos=ii*seq_length
        batches[ii,0] = x[:,pos:pos+seq_length]
        batches[ii,1] = y[:,pos:pos+seq_length]
...
def get_tensors(loaded_graph):
    ...
    return inputs, initial_state, final_state, prob
...
def pick_word(probabilities, int_to_vocab):
    ...
    """
    # TODO: Implement Function
    num = int(np.argmax(probabilities))
    return int_to_vocab[num]
